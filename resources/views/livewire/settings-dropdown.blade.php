<div x-data="{
    ...
    @if ($trigger === 'changelog-sidebar')
        <button wire:click="openWhatsNewModal" type="button" title="{{ __('nav.whats_new') }}" aria-label="{{ __('nav.whats_new') }}"
            ...
            <span class="text-left menu-item-label" :class="collapsed && 'lg:hidden'">{{ __('nav.whats_new') }}</span>
            @if ($unreadCount > 0)
                <span
                    ...
                    aria-label="{{ __('nav.unread_changelog', ['count' => $unreadCount]) }}">
                ...
                <section
                    ...
                    <header class="relative !pr-11">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="mt-0.5 text-xs font-normal text-neutral-500 dark:text-fg-faint">
                                {{ __('nav.product_updates_description') }}
                            </p>
                        </div>
                        </div>
                        ...
                        <button wire:click="closeWhatsNewModal"
                            ...
                            aria-label="{{ __('nav.close_whats_new') }}">
                    ...
                    <div
                        ...
                        <div class="relative w-full max-w-sm">
                            ...
                            <input x-model="search" @input="page = 1" type="search" placeholder="{{ __('nav.search_updates') }}"
                                class="input !pl-9" />
                        </div>
                        <span class="ml-auto hidden text-xs text-neutral-500 sm:block dark:text-fg-faint"
                            x-text="`${filteredEntries.length} ${filteredEntries.length === 1 ? @js(__('nav.update')) : @js(__('nav.updates'))}`"></span>
                    </div>
                    ...
                    <div class="min-h-0 flex-1 overflow-y-auto p-4 scrollbar sm:p-5">
                        ...
                        <div x-show="filteredEntries.length === 0"
                            ...
                            <h3 class="text-sm font-medium text-black dark:text-fg">{{ __('nav.no_updates_found') }}</h3>
                            <p class="mt-1 max-w-sm text-sm text-neutral-500 dark:text-fg-faint">
                                <span x-show="search.trim() !== ''">{{ __('nav.no_releases_match') }}</span>
                                <span x-show="search.trim() === ''">{{ __('nav.no_product_updates') }}</span>
                            </p>
                        </div>
                    </div>
                    <div x-cloak x-show="filteredEntries.length > 0"
                        ...
                        <div
                            ...
                            <button type="button"
                                ...
                                aria-label="{{ __('nav.first_page') }}" title="{{ __('nav.first_page') }}" @click="goToPage(1)"
                                :disabled="page === 1">
                            ...
                            <button type="button"
                                class="flex w-10 items-center justify-center border-r border-neutral-200 text-neutral-500 transition-colors hover:bg-neutral-100 disabled:cursor-not-allowed disabled:opacity-35 dark:border-white/[0.10] dark:text-fg-dim dark:hover:bg-white/[0.06]"
                                aria-label="{{ __('nav.previous_page') }}" title="{{ __('nav.previous_page') }}" @click="goToPage(page - 1)"
                                :disabled="page === 1">
                                <svg class="size-3.5" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                            ...
                            <button type="button"
                                class="flex w-10 items-center justify-center border-r border-neutral-200 text-neutral-500 transition-colors hover:bg-neutral-100 disabled:cursor-not-allowed disabled:opacity-35 dark:border-white/[0.10] dark:text-fg-dim dark:hover:bg-white/[0.06]"
                                aria-label="{{ __('nav.next_page') }}" title="{{ __('nav.next_page') }}" @click="goToPage(page + 1)"
                            ...
                            <button type="button"
                                ...
                                aria-label="{{ __('nav.last_page') }}" title="{{ __('nav.last_page') }}" @click="goToPage(lastPage)"
                                :disabled="page === lastPage">
                                <svg class="size-3.5" viewBox="0 0 24 24" fill="none" aria-hidden="true">
