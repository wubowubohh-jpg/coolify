@can('createAnyResource')
    <form wire:submit="createGitLabApp" class="flex w-full flex-col gap-4">
        <p class="text-[12px] leading-5 text-neutral-500 dark:text-fg-dim">
            {{ __('source.connect_gitlab_oauth_description') }}
        </p>

        <div class="grid gap-4 sm:grid-cols-2">
            <x-forms.input id="name" :label="__('common.name')" required />
            <x-forms.input id="group_name" :label="__('source.group_name')"
                :helper="__('source.group_name_create_helper')"
                :placeholder="__('source.all_accessible_repositories_when_empty')" />
        </div>
        ...
        <div x-data="{
            ...
            <div x-cloak x-show="open" x-collapse.duration.200ms class="border-t border-neutral-200 px-3 py-3 dark:border-white/[0.08]">
                <div class="grid gap-4 sm:grid-cols-2">
                    <x-forms.input id="html_url" :label="__('source.gitlab_url')" required
                        :helper="__('source.gitlab_self_hosted_url_helper')" />
...
@else
    <x-callout type="danger" :title="__('common.insufficient_permissions')">
        {{ __('source.create_gitlab_apps_permission') }}
    </x-callout>
@endcan
