<div class="application-settings-form">
    <form wire:submit="submit" class="flex flex-col gap-6">
        ...
        <x-application.settings-section :title="__('common.database_details')"
            :description="__('common.database_identity_description', ['type' => 'MySQL'])">
            <x-slot:actions>
            ...
            <div class="grid gap-4 lg:grid-cols-2">
                ...
                <div class="lg:col-span-2">
                    <x-forms.input :label="__('common.image')" id="image" required canGate="update" :canResource="$database"
                        :helper="__('common.database_image_helper', ['type' => 'MySQL'])" />
                </div>
            </div>
        </x-application.settings-section>

        <x-application.settings-section :title="__('common.credentials')"
            :description="__('common.credentials_description', ['type' => 'MySQL'])">
            @if ($database->started_at)
                <x-callout type="warning" :title="__('common.keep_credentials_synchronized')">
                    {{ __('common.credentials_sync_description', ['type' => 'MySQL']) }}
                </x-callout>
            @endif
            <div class="{{ $database->started_at ? 'mt-4 ' : '' }}grid gap-4 lg:grid-cols-2">
                ...
                @else
                    <x-forms.input :label="__('common.root_password')" id="mysqlRootPassword" type="password"
                        :required="(bool) $database->started_at" canGate="update" :canResource="$database" />
                @endif
                <x-forms.input :label="__('common.normal_user')" id="mysqlUser" required canGate="update"
                    :canResource="$database" />
                @if ($isPasswordHiddenForMember)
                    <x-forms.input :label="__('common.normal_user_password')" disabled :value="__('common.hidden_admins_only')" />
                @else
                    <x-forms.input :label="__('common.normal_user_password')" id="mysqlPassword" type="password" required
                        canGate="update" :canResource="$database" />
                @endif
                <x-forms.input :label="__('common.initial_database')" id="mysqlDatabase"
        ...
        <x-application.settings-section :title="__('common.configuration')"
            :description="__('common.database_configuration_description', ['type' => 'MySQL'])">
            <x-forms.textarea :label="__('common.custom_database_configuration', ['type' => 'MySQL'])" rows="10" id="mysqlConf"
                canGate="update" :canResource="$database" />
        </x-application.settings-section>
