<div class="application-settings-form">
    <form wire:submit="submit" class="flex flex-col gap-6">
        ...
        <x-application.settings-section :title="__('common.database_details')"
            :description="__('common.database_identity_description', ['type' => 'MongoDB'])">
            <x-slot:actions>
            ...
            <div class="grid gap-4 lg:grid-cols-2">
                ...
                <div class="lg:col-span-2">
                    <x-forms.input :label="__('common.image')" id="image" required canGate="update" :canResource="$database"
                        :helper="__('common.database_image_helper', ['type' => 'MongoDB'])" />
                </div>
            </div>
        </x-application.settings-section>

        <x-application.settings-section :title="__('common.credentials')"
            :description="__('common.credentials_description', ['type' => 'MongoDB'])">
            @if ($database->started_at)
                <x-callout type="warning" :title="__('common.keep_credentials_synchronized')">
                    {{ __('common.credentials_sync_description', ['type' => 'MongoDB']) }}
                </x-callout>
            @endif
            <div class="{{ $database->started_at ? 'mt-4 ' : '' }}grid gap-4 lg:grid-cols-2">
                <x-forms.input :label="$database->started_at ? __('common.initial_username') : __('common.username')"
                    id="mongoInitdbRootUsername" :placeholder="__('common.if_empty_root')"
                ...
                @endif
                <x-forms.input :label="$database->started_at ? __('common.initial_database') : __('common.database')"
                    id="mongoInitdbDatabase" :placeholder="__('common.initial_database_matches_username')"
        ...
        <x-application.settings-section :title="__('common.runtime_and_network')"
            ...
            <div class="grid gap-4 lg:grid-cols-2">
                ...
                <x-forms.input placeholder="3000:27017" id="portsMappings" :label="__('common.port_mappings')"
                    :helper="__('common.port_mappings_helper', ['mapping' => '3000:27017'])"
                    canGate="update" :canResource="$database" />
            </div>
            <div class="mt-4">
        ...
        <x-application.settings-section :title="__('common.configuration')"
            :description="__('common.database_configuration_description', ['type' => 'MongoDB'])">
            <x-forms.textarea :label="__('common.custom_database_configuration', ['type' => 'MongoDB'])" rows="10" id="mongoConf"
                canGate="update" :canResource="$database" />
        </x-application.settings-section>
