    ...
    @if (
        ...
        <div class="flex flex-wrap items-center justify-end gap-2">
            @if (data_get($application_deployment_queue, 'status') === 'queued')
                <x-forms.button class="logs-viewer-deployment-btn" wire:click.prevent="force_start">{{ __('common.force_start') }}</x-forms.button>
            @endif
            <x-forms.button isError class="logs-viewer-deployment-btn logs-viewer-cancel-btn" wire:click.prevent="cancel">{{ __('common.cancel_deployment') }}</x-forms.button>
        </div>
    @endif
</div>
